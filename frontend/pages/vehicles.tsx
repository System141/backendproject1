...
export default function VehiclesPage() {
  ...
  useEffect(() => {
    setLoading(true);
    setError('');
    api.get<Auction[]>('/api/auctions?category_id=1')
  ...
  return (
    <Layout>
      <div className="bg-gradient-to-r from-gray-900 to-gray-800 text-white rounded-2xl p-8 sm:p-12 mb-8">
        <h1 className="text-4xl sm:text-5xl font-black tracking-tight mb-3">{t('nav_vehicles')}</h1>
        <p className="text-gray-300 text-lg max-w-2xl">{t('cat_vehicles_desc')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {!loading && !error && auctions.map(a => (
          <Link key={a.id} href={`/auctions/${a.id}`} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition p-6 flex flex-col">
            ...
            {a.mileage != null && <p className="text-sm text-gray-500">{t('detail_mileage')}: {a.mileage.toLocaleString()} km</p>}
